...
describe("Arenite.Async", function () {
  ...
  it('Sequencial Latch should trigger when executions are completed', function () {
    var callback = jasmine.createSpy('callback');
    var latch = Arenite.Async({config:{debug:false}}).async.seqLatch(['asd'], function () {
      latch.next();
    }, callback);
    latch.next();
    expect(callback).toHaveBeenCalled();
  ...
  it('Sequencial Latch should call handler for each of the values', function () {
    ...
    var latch = Arenite.Async({config:{debug:false}}).async.seqLatch(['asd', 'dsa'], handler, callback);
